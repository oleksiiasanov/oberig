...
The landing page content SHALL be derived from D.VISION SDR's authoritative reference space and SHALL NOT replicate competitor-derived page structure, old landing claims, or prior assumptions.

#### Scenario: Content is re-grounded in D.VISION SDR data
...
### Requirement: Contact CTA Destination
The landing page contact call-to-action SHALL link to the approved D.VISION SDR WhatsApp URL.

#### Scenario: User contacts D.VISION SDR
- **WHEN** the user activates a contact call-to-action
- **THEN** it opens the approved D.VISION SDR WhatsApp URL
...
The landing page SHALL use the real photograph of the D.VISION SDR detector as the primary product media.
...
The landing page SHALL use the supplied D.VISION SDR logo and favicon as brand inputs for the visual identity system.
...
The landing page SHALL present D.VISION SDR as a Ukrainian SDR-based detector/analyzer for analog video signals, not as the old all-band 1050-6040 MHz landing claim.
...
The landing page SHALL describe the market problem using grounded category insights rather than presenting unsupported D.VISION SDR frequency coverage.
...
- **AND** it does not turn category pain points into D.VISION SDR product claims unless confirmed by the technical reference
...
- **THEN** any actual D.VISION SDR frequency ranges remain explicit placeholders unless confirmed in `references/`
...
The landing page SHALL compare D.VISION SDR against the market by criteria grounded in the reference space, not by the old frequency-column table.
...
- **AND** unconfirmed D.VISION SDR figures such as ranges, autonomy, battery, screen size, and price remain placeholders
...
The landing page SHALL present the confirmed D.VISION SDR application modes and device workflows from the reference space.
...
The landing page SHALL present D.VISION SDR as a technical SDR tool for analysis and consultation without tactical workflow instructions.
...
- **AND** the visual system uses complex neutrals, refined borders, glass-like depth where useful, and the D.VISION SDR neon-green accent
