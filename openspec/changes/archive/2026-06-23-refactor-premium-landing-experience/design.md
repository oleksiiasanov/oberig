...
## Decisions
- Decision: Content is sourced from D.VISION SDR's authoritative internal instruction; the current competitor-derived structure is not treated as ground truth.
...
- Decision: Use complex neutrals and the D.VISION SDR neon-green accent, while avoiding a one-note or decorative palette.
...
- Keep the competitor-shaped content: rejected because the content must reflect D.VISION SDR's own product data.
